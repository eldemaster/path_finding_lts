...
def buttons_def():
    b0 = Button((850, 30), "FindAll", 40, "black on white", "white on green", command=pressO)
    b7 = Button((850, 100), "FindOne", 40, "black on white", "white on green", command = pressSpace)
...
def save_to_file(grid, start, PoC, filename="temp.json"):
    ...
    res = {"rows":len(grid),  "barrier":barrier, "start": (start.row,start.col), "ends": ends, "PoC":(PoC.row,PoC.col)} # ADDTO HANDLE ENDS, BUT NEEDS MORE IMPLEMENTATION
...
def make_grid_from_file(filename, width):
    ...
    return grid,start, ends, rows, list(barrier), PoC # NOT PASSED BECAUSE METHOD JUST USES BARRIERS FOR INSTANCE
...
def get_clicked_pos_main(pos, rows, width):
    gap = width //rows
    y,x = pos
    row = y//gap
    col = x//gap
                ...
                if  RunOnce or Run:   
                    ...
                    if SoC < lowbattery:
                        ...
                        if paths:
                            ...
                            RunOnce = False
                            lab1.set("Path length:" + str(int(path_length)))
